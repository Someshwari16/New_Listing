...
export default function UploadSection() {
  ...
  const handleFiles = async (newFiles: File[]) => {
    const validFiles = newFiles.filter(file => {
      ...
      const isUnderSize = file.size <= 50 * 1024 * 1024;
  ...
  const analyzeFiles = async (filesToAnalyze: File[]) => {
    ...
    try {
      // Demo: Create dummy content data from files
  ...
  const analyzeUrl = async () => {
    ...
    try {
      // Demo: Create dummy content data from URL
      const dummyContentData = {
        title: "Trending Product from " + new URL(url).hostname,
        description: "Discover this amazing product that's taking social media by storm. Featured in multiple viral posts.",
        images: [
